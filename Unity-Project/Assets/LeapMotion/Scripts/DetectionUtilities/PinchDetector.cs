...
namespace Leap.Unity {
  ...
  public class PinchDetector : AbstractHoldDetector {
    ...
    protected Quaternion _pinchRotation;
		...
		/* Custom Methods */
		public void addCube(GameObject cubinho)
		{
			cubes.Add(cubinho);
		}
		...
		protected virtual void Update(){
			...
			if (!rigidOff) {
				if (IsPinching) {
					foreach(GameObject cubeinho in cubes){
						cubeinho.GetComponent<Rigidbody> ().isKinematic=true;
						cubeinho.GetComponent<Rigidbody> ().useGravity=false;
					}
				} else {
					foreach(GameObject cubeinho in cubes){
						cubeinho.GetComponent<Rigidbody> ().isKinematic=false;
						cubeinho.GetComponent<Rigidbody> ().useGravity=true;
					}
				}
